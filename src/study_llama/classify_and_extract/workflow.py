...
class ClassifyExtractWorkflow(Workflow):
    ...
    async def classify_file(
        ...
        classifier: Annotated[LlamaClassify, Resource(get_llama_classify)],
    ) -> ClassifiedFileEvent | IngestedFileEvent:
        async with get_db_conn() as db_conn:
            ...
            result = await classifier.aclassify_file_ids(
                rules=class_rules, file_ids=[ev.file_id]
            )
            ...
            if file_type is not None:
                ...
                await querier_files.create_file(
                ...
                return IngestedFileEvent(
                    success=False,
                    error="It was not possible to classify the provided file based on the existing categories",
                )

    @step
    async def extract_file_details(
        self,
        ev: ClassifiedFileEvent,
        extractor: Annotated[LlamaExtract, Resource(get_llama_extract)],
        ctx: Context[WorkflowState],
    ) -> ExtractedFileEvent | IngestedFileEvent:
        ...
        if (data := cast(ExtractRun, result).data) is not None:
            ...
            return ExtractedFileEvent(
                summary=extraction_result.summary, faqs=extraction_result.faqs
            )
        else:
            return IngestedFileEvent(
                success=False,
                error="It was not possible to extract details from the provided file",
            )

    @step
    async def ingest_file_details(
        self,
        ev: ExtractedFileEvent,
        summaries_vdb: Annotated[SummaryVectorDB, Resource(get_vector_db_summaries)],
        faqs_vdb: Annotated[FaqsVectorDB, Resource(get_vector_db_faqs)],
        ctx: Context[WorkflowState],
    ) -> IngestedFileEvent:
        state = await ctx.store.get_state()
        questions = [faq.question for faq in ev.faqs]
        answers = [faq.answer for faq in ev.faqs]
        await faqs_vdb.upload(
            questions, answers, state.username, state.file_type, state.file_name
        )
        await summaries_vdb.upload(
            ev.summary, state.username, state.file_type, state.file_name
        )
